...
try {
    ...
    for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
        if ($task -eq 'residents') {
            ...
            & $python (Join-Path $base 'fetch_daily_residents.py') $date *>> $log
            ...
            & $python (Join-Path $base 'fetch_income_flow_range.py') $start $end *>> $log
        } elseif ($task -eq 'board') {
            Log ('AMS board update, attempt {0}/{1}' -f $attempt, $maxAttempts)
            & $python (Join-Path $base 'update_ams_board.py') *>> $log
        } else {
